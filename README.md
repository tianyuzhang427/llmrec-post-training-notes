...
## 模型融合实验：模型 A × 模型 B 正交残差

两个输入模型均为同一 `OneReason-0.8B-pretrain-competition` 底座上的
`rank=16, alpha=32` LoRA。记模型 A 和模型 B 合入底座后的任务增量为
`ΔA`、`ΔB`。二者全局余弦相似度为 `0.8105`，因此没有直接相加，而是
对每个同名 tensor 从 `ΔB` 中剔除与 `ΔA` 重合的投影：

```text
R_B,k = ΔB_k - (<ΔB_k, ΔA_k> / ||ΔA_k||²) ΔA_k
W_fused,k = W_A,k + 0.25 R_B,k
```

| 模型 | 总分 | 懂物料 | 用户-1 | 用户-2 | 视频 | 商品 | 广告 | 直播 | 世界 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 模型 A | 1.0014 | 0.2453 | 0.0806 | 0.0356 | 0.0768 | 0.1360 | 0.1498 | 0.1152 | 0.1621 |
| 模型 B | 0.9954 | 0.2760 | 0.0727 | 0.0377 | 0.0768 | 0.1156 | 0.1470 | 0.1098 | 0.1599 |
| 正交残差融合，`λ=0.25` | **1.0206** | 0.2453 | **0.0833** | **0.0383** | **0.0960** | **0.1360** | 0.1484 | 0.1116 | 0.1617 |
| 正交残差融合，`λ=0.375` | 0.9983 | 0.2453 | 0.0873 | 0.0369 | 0.0672 | **0.1462** | 0.1428 | 0.1116 | 0.1610 |

融合结果相对模型 A 提升 `+0.0192`。物料没有从 `0.2453` 提升到模型 B 的
`0.2760`；实际增益来自两个用户分项和视频推荐，代价是广告、直播和世界
轻微下降。因此这里的“融合有效”仅指该次总分与分项结构获得正向结果，不表示已经
实现逐能力无损拼接。

`λ=0.375` 使用相同输入和方法，仅改变残差系数，线上总分回落到 `0.9983`。
相对 `λ=0.25`，用户-1和商品继续提高，但视频下降 `-0.0288`、广告下降
`-0.0056`，最终总分下降 `-0.0223`。因此这不是可以通过持续增大系数获得单调
提升的方向；当前已验证甜点为 `λ=0.25`，没有依据继续尝试 `λ=0.5`。
