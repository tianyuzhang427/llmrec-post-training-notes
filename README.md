...
1. **难样本预算。** 调整 item-token loss 权重和 focal `γ` 时，线上分数出现了超出预期的变化。由此推测，seed dataset 中存在大量简单、重复的样本，占用了训练预算，却没有提供足够的新信息。基于这一判断，我们清洗了“懂推荐”的 Video domain，将视频样本减少约 50%，其他推荐域保持不变。线上结果验证了这一方向有效；但继续减少到 35% 后收益转负，说明有效的是适度去除冗余，而不是数据越少越好。
